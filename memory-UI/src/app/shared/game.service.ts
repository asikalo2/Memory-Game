...
export class GameService {
  ...
  getGameCode(userCode: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      this.stompClient.connect({}, () => {
        this.stompClient.subscribe('/topic/user' + userCode, (payload) => {
          var data = JSON.parse(payload.body);
          console.log(data);
          if (!data || data.gameCode === null) {
            reject();
          } else {
            this.stompClient.subscribe('/topic/room' + data.gameCode, (payload) => {
              var game = JSON.parse(payload.body);
        ...
        this.stompClient.send("/app/memory/findRoom", {},
          JSON.stringify({ userCode: userCode, username: GameService.gameStarter.username })
        );
      });
    });
  }
  ...
  subscribeToGameStarter(gameCode) {
    ...
    this.stompClient.connect({}, function (frame) {
      that.stompClient.subscribe('/topic/room' + gameCode, (payload) => {
        //konekcija na igru
      },
        error => {
          console.log('Subscribe: error: ' + error)
      ...
      that.stompClient.send("/app/memory/startGame", {},
        JSON.stringify({ userCode: GameService.gameStarter.users[0].code, username: GameService.gameStarter.users[0].username })
      );
    });
  }
